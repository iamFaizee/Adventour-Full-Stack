...
function Tourdetail() {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:5000/package/details/${_id}`)
          ...
          <div className={styles.posi}>
            <Flex bg={theme ? '#191b1d' : 'white'} direction={'column'} w={{ base: '100%', md: '110%', lg: '350px' }} boxShadow={'md'} border={'1px solid silver'} p={'20px'} mt={{ base: '40px', md: '10px' }} ml={{ base: '0px', md: '0px', lg: '40px' }} borderRadius={'20px'}>
              ...
              <Flex mb={'20px'} w={{ base: '100%', md: '100%', lg: '300px' }} alignItems={'center'} pt={'10px'} pb={'10px'}>
                <Link to={`/payment/${location}/${_id}`}><Button h={'45px'} w={{ base: '85%', md: '85%', lg: '230px' }} colorScheme='none' fontWeight={'700'} bg={theme ? "#3DC6EF" : "#008cc9"} color={theme ? 'white' : 'blackAlpha.700'} borderRadius={'40px'}>Check Availability</Button></Link>
                <Spacer />
                <IconButton onClick={()=>{
                  if(!wish){
                    toast({
